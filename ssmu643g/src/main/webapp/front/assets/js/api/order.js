...
function ordersList(page, limit, status, callback) {
	...
	if(status=="全部"){
		httpJson(`orders/list?page=${page}&limit=${limit}&userid=${userid}`, 'GET', '', callback);
	}else{
		httpJson(`orders/list?page=${page}&limit=${limit}&status=${status}&userid=${userid}`, 'GET', '', callback);
	}
